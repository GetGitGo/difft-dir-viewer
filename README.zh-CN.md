...
本仓库为 **独立项目**。请单独安装或构建 `difft`（见下文）；Viewer 以子进程方式调用。
...
# 或从 https://github.com/wilfred/difftastic 构建后设置：
...
行高、gutter 宽度与水平滚动步长会随字号同步缩放。代码区使用系统默认 **monospace** 等宽字体、粗体显示。
...
**Changed files** 侧栏：路径可横向滚动（状态列固定）；点击侧栏后，上述滚动快捷键作用于文件列表，点击代码区后作用于 diff 代码。字号快捷键在侧栏或代码区聚焦时均可用。
